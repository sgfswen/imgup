 ...
 * Protected: Yes
 * Purpose: Process an upvote for a specific picture from an authenticated account
...
if ($request->isPost()) {
  ...
  if ($authHeader) {
    ...
    if ($authenticated){
        ...
        $sql = "SELECT * FROM UPVOTES WHERE user=:user AND photo=:photo";
        $stmt= $db->prepare($sql);
        $stmt->bindValue(':user', $userid);
        $stmt->bindValue(':photo', $photoid);
        $stmt->execute();
        $rows = $stmt->rowCount();
        // User has already upvoted, send 400
        if ($rows>0) { 
        ...
        // User has not upvoted
          ...
          // Insert Upvote into UPVOTES (for checking above)
          $sql = "INSERT INTO UPVOTES (user,photo) VALUES (:user, :photo)";
          $stmt= $db->prepare($sql);
          $stmt->bindValue(':user', $userid);
          $stmt->bindValue(':photo', $photoid);
          $stmt->execute();
          ...
          $sql = "UPDATE UPLOADS SET up=up+1 WHERE id=:photoid";
          $stmt= $db->prepare($sql);
